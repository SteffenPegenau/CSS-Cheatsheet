...
\begin{tikzpicture}
  \draw (0,1) node[anchor=north west,font=\Huge\fontfamily{phv}\selectfont]{Cheatsheet: Computersystemsicherheit}
  (.1cm,.3cm) -- (28.2cm,.3cm)
  ...
  ++(0, -2.9cm) node[below right,text width=\breiteErsteSpalte,rounded corners,fill=gray!15,inner sep=1ex,drop shadow]
  {
    ...
    Nicht injektive Abbildung, die Urbildbildbereich auf erheblich kleineren Bildbereich abbildet. Einwegfunktion.\\
...
\begin{tikzpicture}
  ...
  (linksoben)
  {
   ...
   $n > 1 \Rightarrow a^{\phi(n)} \equiv 1(\mod n) \forall a \in \mathbb{Z}_n^*$ 
  }
  ++(0,-1.3cm) node[below right,text width=\breiteErsteSpalte,rounded corners,fill=gray!15,inner sep=1ex,drop shadow]
  {
    \platz
    \subsubsection*{Spezialfall: kleiner Satz von Fermat}
    Primzahl $p>1$ so gilt für jede Zahl $x$ mit $ggT(x,p) = 1:$ \\
    $$x^{p-1} \equiv 1 (\mod n)$$
    In jeder endl. Gruppe $M$ gilt: $x^{ord(M)} = 1 \forall x \in M$
  }
  ++(0,-3cm) node[below right,text width=\breiteErsteSpalte,rounded corners,fill=gray!15,inner sep=1ex,drop shadow]
  {
    \platz
    \subsubsection*{Korrektheit von ElGamal}
    Bekannt: $E[(g,h),m] = (c_1, c_2) := (g^k, mh^k)$ \\
    Zu zeigen: $D(E(m)) = m \quad \forall m \in \mathbb{Z}_p^*$ \\
    Beweis:
    \begin{align}
     D(E(m)) &= D(g^k, mh^k) \\
     &\equiv mh^k \cdot (g^k)^{-a} \mod p \\
     &\equiv mh^k \cdot g^{k(p-1-a)} \mod p \\
     &\equiv mg^{ak} \cdot g^{k(p-1)-ak} \mod p \\
     &\equiv m\cdot g^{k(p-1)} \mod p \\
     &\equiv m \mod p
    \end{align}

  }
  ++(0, -5.5cm) node[below right,text width=\breiteErsteSpalte,rounded corners,fill=gray!15,inner sep=1ex,drop shadow]
  {
    \platz
    \subsection*{RSA}
    Allgemein: \\
    \begin{tabular}{ll}
     ...
     2. & Setze $n = p\cdot q$ \\
     3. & Wähle zufällig $(e,d)$ mit \\
	& $ed \equiv 1 mod (\varphi(n))$, mit $\varphi(n) = (p-1)(q-1)$
    \end{tabular}
    Public key: $pk = (e,n)$ \\
    Private key: $sk = (d,n)$ \\
    (Public und private key sind invers zueinander)
    Ver-/Entschlüsselung: \\
    $E(pk, m) = m^e\mod n$ \\
    $D(sk, c) = c^d \mod n$\\
    RSA Signatur: \\
    $sign(sk,m) = h(m)^d \mod n$ \\
    $verify(pk, m,s): [s^e = h(m) ?]$
...
;
  ...
  {
    \platz
    \subsubsection*{Zeigen, dass Chiffriersystem definiert ist}
    Zu zeigen ist: $D[E(m_1, \dots, m_n)] = m_1,\dots,m_n $
  }
  ++(0,-1.3cm) node[below right,text width=\breiteZweiteSpalte,rounded corners,fill=gray!15,inner sep=1ex,drop shadow]
  {
    \platz
    \subsubsection*{Ansätze zur Userauthentifizierung}
    ...
    - cued recall-based (Mit Abrufhilfe, Sicherheitsfrage) \\
      PassPoints \\
    - Recognition-based (Wiedererkennen) \\
    by ownership (smart card, token, SIM)\\
    by inherence (Biometrie, Verhalten)\\

  }
  ++(0,-4cm) node[below right,text width=\breiteZweiteSpalte,rounded corners,fill=gray!15,inner sep=1ex,drop shadow]
  {
    \platz
    \subsubsection*{Features of authentication schemes}
    Security \\
    ...
    - Recordability \\
      Angreifbar über: \\
      Social Engineering (Deception), Theft (Unsecured Record) \\
    Usability \\
    - Compatibility\\
    - Costs \\
    - Maturity \\
  ...
  ++(0,-6.7cm) node[below right,text width=\breiteZweiteSpalte,rounded corners,fill=gray!15,inner sep=1ex,drop shadow]
  {
    \platz
    \subsubsection*{Mandatory Access Control (MAC)}
    Systembestimmte (regelbasierte) Festlegung von Sicherheitseigenschaften, die benutzerdefinierte
    Rechte dominieren. Bell-La Padula Modell erweitert Matrixbasiertes Zugriffskontrollsystem. \\
    Rechte $R=\{ read, append, read-write \}$ \\
    Sicherheitsklassen $SC = (A, B)$ mit $A$ total geordnete Sicherheitsmarken,
    mit $B$ Menge von Kategorien/Personen. \\
    Jedes Subjekt hat Clearance: $SC(s) \in SC$ \\
    Jedes Objekt hat Classification: $SC(o) \in SC$ \\
    Regeln: \\
    - Simple-security-Property/no read up-Regel. Lesen nur erlauben, wenn \\
      $read \in M(s,o) \land SC(s) \geq SC(o)$ \\
  ...
  {
    \platz
    \subsubsection*{Usability aspects of knowledge based authentication}
    \begin{tabular}{l l l}
    Effectiveness & Objective & Success-/Errorrate \\
    Efficiency & Objective & Time needed per attempts, attempts needed \\
    Satisfaction & Subjective & Questionnaires (SUS), Recommendations
    \end{tabular}
  ...
  {
    \platz
    \subsubsection*{Discretionary Access Control (DAC) - Fahrschein}
    ...
    Nachteile: dynamische Rechte schlecht abbildbar, Vergabe/Rücknahme von Rechten
    relativ komplex \\
  ...
  ++(0,-3.6cm) node[below right,text width=\breiteDritteSpalte,rounded corners,fill=gray!15,inner sep=1ex,drop shadow]
  {
    ...
    Benutzer - Rollen: $sr: S \rightarrow 2^R$ \\
    Rolle - Rechte: $pr: R \rightarrow 2^P$ (Rolle bestimmt Rechte)\\
    Sitzung: $session \subseteq S \times 2^R$ \\
    Für alle Paare $(s,rl)\in session: \quad rl \subseteq sr(s)$ \\
    $rl$: aktive Rollen von $s$\\
    Invarianten: \\
    $\forall s \in S: R_j \in session(s) \Rightarrow sr(s)$: \\
    Subjekt darf nur Rollen haben, in denen es Mitglied ist \\
    $\forall s \in S: \underline{exec(s,p)}_{\text{hat s p?}} \Rightarrow R_j \in R: R_j \in session(s) \wedge p \in pr(R_j)$: \\
    Subjekt hat nur Rechte der aktiven Rolle \\
    $\forall s \in S: exec(s,p) \Rightarrow \exists R_i \in R, R_i  \in session(s) \land (p \in pr(R_i) \lor \exists R_j: (\exists R_j: (R_j < R_i \wedge p \in pr(R_j))))$ \\
    ...
    $\forall R_i, R_j \in R, \forall s \in S:$ \\ $(s\in member(R_i) \land s\in member(R_j) \land \{R_i, R_j\} \subseteq active(s))$  \\
    $ \Rightarrow (R_i, R_j) \not\in SSD$ \\
    $active(s) = \{R_i \vert \exists R_s \in R \land (s,R_s) \in session \land R_i \in R_s \} $
  }
  ;
\end{tikzpicture}
